...
const SpecifySalary = () => {
  ...
  const { id } = useParams();
  useEffect(() => {
    axios.get('http://localhost:8000/auth/employee3/' + id)
    .then(result => {
      if(result.data.Status){
        const filteredEmployees = result.data.Result.filter(d => d.eid === Number(id));
    ...
    return (
      <React.Fragment key={w.shiftID}>
        <tr>
          ...
          <td>{Salary}</td>
          <td>
          <Link to = {"/dashboard/manageEmpSalary/" + w.eid} className='btn btn-success'>Manage</Link>
          </td>
          <td>
